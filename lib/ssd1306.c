...
// Exemplo de versão modificada para suportar minúsculas
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y)
{
    uint8_t *font_ptr = NULL;

    // Se o caractere for uma letra maiúscula
    if (c >= 'A' && c <= 'Z') {
        font_ptr = font + ((c - 'A' + 11) * 8);  // Maiúsculas começam no índice 11
    }
    // Se o caractere for um número
    else if (c >= '0' && c <= '9') {
        font_ptr = font + ((c - '0' + 1) * 8);  // Números começam no índice 1
    }
    // Se o caractere for uma letra minúscula
    else if (c >= 'a' && c <= 'z') {
        font_ptr = font + ((c - 'a' + 11) * 8);  // Minúsculas começam no índice 36 (após A-Z e 0-9)
    } else {
    ...
    for (uint8_t i = 0; i < 8; ++i) {
        uint8_t line = font_ptr[i];  // Linha atual do caractere na tabela de fontes
        for (uint8_t j = 0; j < 8; ++j) {
            // Verifica o bit da linha (da esquerda para a direita)
            if (line & (1 << j)) {  // Lê da esquerda para a direita
                ssd1306_pixel(ssd, x + i, y + j, true);  // Acende o pixel
            } else {
                ssd1306_pixel(ssd, x + i, y + j, false); // Apaga o pixel
            }
        }
    }
}
